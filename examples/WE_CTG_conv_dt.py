...
if __name__ == "__main__":
    ...
    data_file = data_dir / "data_swe.yaml"
    ...
    for i, dt in enumerate(ddt):
        ...
        config.numerics.t_slab_size = ddt[i]

        # Compute
        ctg_solver = CTGSolver(config.numerics)
        sol_slabs, time_slabs, space_time_fe, total_n_dofs = ctg_solver.run(config.physics)
    ...
    plt.figure()
    for i, dt in enumerate(ddt):
        tt = np.linspace(config.physics.start_time, config.physics.end_time, int(1 / dt))
